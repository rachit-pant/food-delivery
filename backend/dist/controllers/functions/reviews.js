...
export const getItemsReviews = expressAsyncHandler(async (req, res) => {
    ...
    const realReviews = getItemsReviews.filter((item) => !review.has(item.order_id));
    res.status(200).json(realReviews);
});
export const postItemReviews = expressAsyncHandler(async (req, res) => {
...
export const showReviews = expressAsyncHandler(async (req, res) => {
    const resId = Number(req.params.restaurantId);
