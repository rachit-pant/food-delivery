...
const StripeWebhooks = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        throw new BetterError("webhook error", 400, "WEBHOOK_ERROR", "Subscription Error");
    }
    console.log(event);
    switch (event.type) {
        case "checkout.session.completed": {
            ...
            if (!session.subscription) {
                ...
                if (!userId || !restaurant_id || !addressId) {
                    throw new BetterError("user id or restaurant id or address id not found", 400, "USER_ID_OR_RESTAURANT_ID_OR_ADDRESS_ID_NOT_FOUND", "Subscription Error");
                }
                ...
                const newOrder = await prisma.orders.create({
                    data: {
                        ...
                        payment_status: session.payment_status === "paid" ? "paid" : "not_paid",
                        status: "preparing",
                        restaurant_id,
                        order_items: {
                ...
                const socketId = onlineUsers.get(ownerId?.user_id) || [];
            ...
            if (!plan) {
                throw new BetterError("no plan found", 404, "NO_PLAN_FOUND", "Subscription Error");
            }
            const userId = Number(session.client_reference_id);
        ...
        case "invoice.payment_succeeded": {
            ...
            if (!user) {
                throw new BetterError("User not found", 404, "USER_NOT_FOUND", "Subscription Error");
            }
