...
export const regUser = asyncHandler(async (req, res) => {
    ...
    console.log('req.body', req.body, full_name, email, phone_number, password, receivedToken);
    if (!email || !password) {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (!invite) {
            throw new BetterError('Invalid or missing invite', 400, 'INVALID_INVITE', 'Invite Error');
        }
        if (invite.expiresAt && invite.expiresAt < new Date()) {
            throw new BetterError('Invite has expired', 400, 'EXPIRED_INVITE', 'Invite Error');
        }
        console.log('error');
...
export const login = asyncHandler(async (req, res) => {
    ...
    if (!validPassword) {
        throw new BetterError('Invalid email or password', 401, 'BAD_CREDENTIALS', 'User Error');
    }
    const tokenHash = hashToken(receivedToken);
