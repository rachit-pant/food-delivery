...
const addStaffInvites = expressAsyncHandler(async (req, res) => {
    const { email, franchiseId, role } = req.body;
    if (!email || !franchiseId || !role) {
        throw new BetterError('Invalid Request', 400, 'INVALID_REQUEST', 'Missing required fields');
    }
    ...
    if (existingInvite) {
        throw new BetterError('Invite already sent', 400, 'DUPLICATE_INVITE', 'This user already has a pending invite.');
    }
    ...
    if (existingInviteFranchise) {
        throw new BetterError('In a franchise already sent', 400, 'DUPLICATE_INVITE', 'This user already has a pending invite.');
    }
...
`;
    try {
        ...
        throw new BetterError('Failed to send invite email', 500, 'EMAIL_ERROR', err.message);
    }
