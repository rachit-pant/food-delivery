...
const Dashboard = expressAsyncHandler(async (req, res) => {
    ...
    if (!planId) {
        throw new BetterError("Upgrade Plan", 404, "UPGRADE_PLAN", "Subscription Error");
    }
    if (allowedPlans.includes(planId.plan_id)) {
        try {
            ...
            const avgRating = restaurant.reduce((total, restaurant) => total + (restaurant.rating ?? 3), 0) / restaurant.length;
            ...
            throw new BetterError("Dashboard not found", 500, "DASHBOARD_NOT_FOUND", "Dashboard Error");
        }
    }
    else {
        throw new BetterError("Upgrade Plan", 500, "UPGRADE_PLAN", "Subscription Error");
    }
});
