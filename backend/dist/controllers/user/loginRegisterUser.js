...
export const regUser = asyncHandler(async (req, res) => {
    ...
    if (email === '' || password === '') {
        throw new BetterError('enter correct details', 400, 'BAD_REQUEST', 'User Error');
    }
    let hash;
    try {
    ...
    try {
        ...
        if (err.code === 'P2002') {
            if (err.meta?.target[0] === 'email') {
                throw new BetterError('Email already exists', 409, 'P2002', 'Prisma Error');
            }
            else if (err.meta?.target[0] === 'phone_number') {
                throw new BetterError('Phone number already exists', 409, 'P2002', 'Prisma Error');
            }
            else {
...
export const login = asyncHandler(async (req, res) => {
    ...
    if (!email || !password) {
        throw new BetterError('Email and password are required', 400, 'BAD_REQUEST', 'User Error');
    }
    let user;
    try {
    ...
    if (!(await bcrypt.compare(password, user.password))) {
        throw new BetterError('Invalid email or password', 401, 'BAD_REQUEST', 'User Error');
    }
    ...
    try {
        ...
        throw new BetterError('User not updated', 404, 'USER_NOT_UPDATED', 'User Error');
    }
});
